...
namespace AiAlgorithms.Algorithms
{
    public class StatValue
    {
        ...
        {
            return $"mean value {Mean.ToString(CultureInfo.InvariantCulture)}\n" +
                   $"standart deviation {StdDeviation.ToString(CultureInfo.InvariantCulture)}\n" +
                   $"min {Min.ToString(CultureInfo.InvariantCulture)}\n" +
                   $"max {Max.ToString(CultureInfo.InvariantCulture)}\n" +
                   $"confidence interval {ConfIntervalSize.ToString(CultureInfo.InvariantCulture)}\n" +
                   $"run counts {Count}";
        }
